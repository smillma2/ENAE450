...
class SolveMaze(Node):
    ...
    def get_scan_data(self, data):
        ...
        self.front = min(data.ranges[320:380])
        
        # ON REAL ROBOT, CHANGE TO PROPER DATA RANGES
        data_left = [12.0 if x == float('inf') else x for x in data.ranges[450:630]]
        data_right = [12.0 if x == float('inf') else x for x in data.ranges[90:270]]
    ...
    def timer_callback(self):
        ...
        # if self.e_dist_r > self.side_threshold:
        #     message.linear.x = 0.0
        #     message.angular.z = -0.2
        else:
            # reset front threshold after hysteresis, if it happened
            self.front_threshold = 0.34
            # move forward at constant rate
            message.linear.x = 0.17
            ...
            message.angular.z = (aim_angle * math.pi/360) + wall_dist_modifier + 0.09
